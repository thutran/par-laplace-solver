...
int main(int argc, char *argv[]) 
{
    ...
    if (my_PE_num==0){
        ...
        if (fsum)
            fprintf(fsum, "mpi\t%d\t%d\t%d\t%f\t%g\n", npes, max_iterations, iteration - 1, dt_global, elapsed_time.tv_sec + elapsed_time.tv_usec / 1000000.0);
        
    }

    if (my_PE_num == npes - 1 && !quiet)
    {
